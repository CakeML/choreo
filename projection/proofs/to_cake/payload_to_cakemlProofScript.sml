...
Triviality do_eval_res_swap_clock:
  ∀(s0:α state) s l res ck.
  do_eval_res l s0 = (s,res)
  ⇒ do_eval_res l (s0 with clock := ck) = (s with clock := ck,res)
Proof
  rw[do_eval_res_def] \\ gs[AllCaseEqs()]
QED

Triviality do_eval_res_same_clock:
  ∀(s0:α state) s l res ck.
  do_eval_res l s0 = (s,res)
  ⇒ s0.clock = s.clock
Proof
...
Proof
  ...
  >- ((* App *) gvs[AllCaseEqs(), find_evalform ‘App _ _’] >>
      ...
      >- (qabbrev_tac ‘d2 = s00.clock - s.clock’ >>
          ...
          first_x_assum (qspec_then ‘ck + d2’ mp_tac) >> simp[] >> rw[] >>
          qmatch_asmsub_abbrev_tac ‘result_bind ff _’ >> cases_on ‘ff’ >> gs[] >>
          drule_then (qspec_then ‘ck + d2’ assume_tac) do_eval_res_swap_clock >>
...
Proof
  ...
  >- (gs[cpEval_valid_Send] >>
      ...
      pop_assum (irule_at (Pos hd)) >>
      irule_at (Pos hd) (small_big_exp_equiv |> iffRL |> cj 1) >>
      ‘cSt0.eval_state = NONE’ by gs[cpEval_valid_def] >> simp[] >>
      irule_at Any (iffRL bigClockTheory.big_clocked_unclocked_equiv) >>
      simp[funBigStepEquivTheory.functional_evaluate] >>
  ...
  >- ((* second SEND case *) gs[cpEval_valid_Send] >>
      ...
      qexists_tac ‘cSt0 with <| ffi := new_ffi; refs := new_refs |>’ >>
  ...
  >- ((* receiveloop - finishing*) all_tac >>
      ...
      qexists_tac ‘ARB with eval_state := NONE’ >>
      ...
      qexists_tac ‘ARB with eval_state := NONE’ >> simp[] >>
      irule_at Any LESS_EQ_REFL >> simp[EXstrefsffi] >>
      qmatch_goalsub_abbrev_tac ‘triR _ (_, (new_refs, new_ffi), _, _)’ >>
      CONV_TAC (pull_namedexvar_conv "cSt") >>
      qexists_tac ‘cSt0 with <| ffi := new_ffi; refs := new_refs |>’ >>
      simp[Abbr‘new_refs’, Abbr‘new_ffi’] >>
      irule_at (Pos hd) triR_REFL >> simp[] >>
  ...
  >- ((* receiveloop - continuing *) all_tac >>
      ...
      CONV_TAC (pull_namedexvar_conv "newrefs") >> qexists_tac ‘[]’ >> simp[] >>
      first_x_assum $ irule_at (Pos hd) >> simp[continue_def] >>
      MAP_EVERY (CONV_TAC o pull_namedexvar_conv) ["s0","t"] >>
      qexists_tac ‘ARB with eval_state := NONE’ >>
      qexists_tac ‘ARB with eval_state := NONE’ >> simp[] >>
      (* If (finalv "buff") ... *)
      irule_at Any triR_step1 >>
...
Theorem network_NPar_forward_correctness:
  ∀conf s c n p s' c' n' cSt0 vs cvs env0.
    trans conf (NPar (NEndpoint p s c) n) LTau (NPar (NEndpoint p s' c') n') ∧
    (∀nd. nd ∈ network_nodenames (NEndpoint p s c) ⇒
          ffi_has_node nd cSt0.ffi.ffi_state) ∧
...
Proof
  ...
  >- (drule simulation >>
      ...
      >- (gs[DISJ_IMP_THM,FORALL_AND_THM,can_match_def,
             ...
             ELIM_UNCURRY]) >>
      rpt strip_tac >> goal_assum drule >>
      simp[] >>
...
Theorem ffistate_selfup:
  ffi with ffi_state := ffi.ffi_state = ffi
Proof
  simp[ffi_state_component_equality]
QED

Theorem ffi_irrel_smallsteps' =
        SRULE [to_small_st_def, FAstrefsffi] ffi_irrel_smallsteps
        |> SPEC_ALL
        |> Q.INST [‘ffi'’ |-> ‘ffi1 with ffi_state := ffis1’,
                   ‘ffi2’ |-> ‘ffi1.ffi_state’]
        |> SRULE[ffistate_selfup]
...
Theorem network_NPar_forward_correctness':
  ∀conf s c n p s' c' n' cSt0 vs cvs env0.
    ...
    ∃env cSt env' e' l' sst sst' vs'.
      stepr꙳ (env0, smSt cSt0, Exp (compile_endpoint conf vs c), [])
             (env', sst, e', l') ∧
      stepr꙳ (env, smSt cSt, Exp (compile_endpoint conf vs' c'), [])
             (env', sst', e', l') ∧
...
Proof
  ...
  >- (gvs[cpEval_valid_def, ffi_state_cor_def]) >>
  strip_tac >>
  gvs[triR_def, to_small_st_def]
  >- (drule_then assume_tac NRC_RTC >>
      drule_at (Pos last)
               (SRULE [to_small_st_def, FAstrefsffi] ffi_irrel_smallsteps) >>
      simp[] >>
      disch_then (resolve_then (Pos hd) mp_tac (iffLR ffi_eq_SYM)) >>
      disch_then drule >> simp[ffistate_selfup] >> impl_tac
      >- gvs[cpEval_valid_def] >>
      strip_tac >> goal_assum drule >> simp[] >>
      irule_at (Pos hd) RTC_REFL >> simp[]>> metis_tac[ffi_eq_SYM]) >>
  dxrule NRC_RTC >>
  disch_then (mp_then (Pos last) mp_tac
              (SRULE [to_small_st_def, FAstrefsffi] ffi_irrel_smallsteps)) >>
  simp[] >>
  disch_then (resolve_then (Pos hd) mp_tac (iffLR ffi_eq_SYM)) >>
  disch_then drule >> simp[ffistate_selfup] >> impl_tac
  >- gvs[cpEval_valid_def] >> strip_tac >> goal_assum drule >> simp[] >>
  dxrule_then assume_tac NRC_RTC >> goal_assum drule >> simp[] >>
  metis_tac[ffi_eq_SYM]
QED
