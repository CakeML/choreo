...
∧ (written_var_names_endpoint (Letrec dv vars e1) =
   written_var_names_endpoint e1 ++ MAP FST (FILTER ($¬ o SND) vars))
...
∧ (written_var_names_endpoint_before dn (Letrec dv vars e1) =
   if dn = dv then
     ...
     written_var_names_endpoint_before dn e1 ++ MAP FST (FILTER ($¬ o SND) vars))
...
Definition compile_endpoint_def:
    (compile_endpoint uw ar payloadLang$Nil = payloadLang$Nil) ∧
    (compile_endpoint uw ar (Send p v n e) = Send p v n (compile_endpoint uw ar e)) ∧
    (compile_endpoint uw ar (Receive p v l e) = Receive p v l (compile_endpoint (FILTER ($≠ v) uw) ar e)) ∧
    (compile_endpoint uw ar (IfThen v e1 e2) =
     IfThen v (compile_endpoint uw ar e1) (compile_endpoint uw ar e2)) ∧
    (compile_endpoint uw ar (Let v f vl e) =
     Let v f vl (compile_endpoint (FILTER ($≠ v) uw) ar e)) ∧
    (compile_endpoint uw ar (Letrec dn vars e1) = Nil (* Source term should use fixpoint style *)) ∧
    (compile_endpoint uw ar (FCall dn vars) = Nil (* Source term should use fixpoint style *)) ∧
    (compile_endpoint uw ar (Fix dn e) =
     let vars = nub'(written_var_names_endpoint e);
         e' = compile_endpoint (FILTER (λv. ¬MEM v vars) uw) ((dn,vars)::ar) e
     in
       Letrec dn (MAP (λv. (v, ¬MEM v uw)) vars) e'
    ) ∧
    (compile_endpoint uw ar (Call dn) =
...
∧ compile_network (payloadLang$NEndpoint p s e) =
  let
  ...
  in
    NEndpoint p s (compile_endpoint undefined_writes [] e)
End
